...
class DM_Channel_Commands(Channel_Commands):
  ...
  async def process_command(self, message):
    ...
    game_title = message.content in Server_Bot_Functions.get_games()
    command = message.content in self.child_commands

    if not(game_title or command):
        return
    
    if game_title:
        client_bot = Server_Bot_Functions.get_client_bot_for_game(message.content)
        server_message = {
